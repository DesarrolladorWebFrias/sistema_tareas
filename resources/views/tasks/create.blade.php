...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Crear Nueva Tarea</h1>

        <form action="{{ route('tasks.store') }}" method="POST" class="bg-white p-8 rounded-lg shadow-lg">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="due_date" id="due_date" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('due_date') border-red-500 @enderror" value="{{ old('due_date') }}">
                @error('due_date')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
